...
> **Historical report — superseded by `docs/reviews/sidebar-pr3-final-review.md`.**
> This document captured the state at the original sidebar RBAC review. Some
> claims below are stale against the current branch and are annotated inline:
> the user menu component was renamed `dashboard-user-menu.tsx` →
> `dashboard-footer-menu.tsx`, the latent role fallback it flagged as "removed"
> was subsequently removed by the PR #3 follow-up, the duplicate `ROLE_LABEL`
> was hoisted to `src/constants/dashboard.ts`, and the format baseline drifted.
> Prefer the PR #3 final review for current state.
...
1. **Role fallback `role ?? 'member'` contradicted the plan and user constraint. (Historical — now resolved.)**
   `src/layouts/dashboard-layout.tsx:26` — `DASHBOARD_NAV_BY_ROLE[role ?? 'member']` and `src/pages/dashboard-index.tsx:23` — `role === 'admin' ? <AdminPage /> : <DashboardPage />`. The plan says replace pathname detection with session role, and the user constraint says "do not invent role fallback." In practice the dashboard tree is behind `RequireAuth` (`src/app.tsx:68-73`), so `role` is never null at render; the fallback was latent, not observable. Current code resolves menus with `role ? DASHBOARD_NAV_BY_ROLE[role] : []` — no invented default.

### Minor

1. **`AlertDialog` → `DropdownMenu` nesting order is inverted from Sidebar 07.** `src/components/dashboard/shared/dashboard-footer-menu.tsx:74-136` wraps `DropdownMenu` inside `AlertDialog.Root`, and the logout item is `<DropdownMenuItem render={<AlertDialogTrigger>…} />`. Base UI closes the menu on item click (`closeOnClick` default `true`), then the trigger fires. Works because the dialog is portaled separately, but the reference `nav-user.tsx` composes the dialog as a sibling of the menu, not nested around it. If dialog-open flakiness appears, hoist `AlertDialog` state and open it from `DropdownMenuItem.onClick` (`closeOnClick={false}` or `onClick`), matching the reference shape more closely.
2. **Duplicate logout error copy.** `dashboard-footer-menu.tsx:65` — `toast.error('Gagal keluar. Coba lagi.')`; acceptable, but the 401 path relies on `ApiError` identity from `src/lib/api/client.ts:3`. No duplicate logout logic found elsewhere (good).
3. **`ROLE_LABEL` map was duplicated. (Historical — now resolved.)** `dashboard-footer-menu.tsx` and `src/pages/dashboard-profile.tsx` both defined `{ admin: 'Admin Dispora', member: 'Pemuda Wirausaha' }`. Current code imports the shared `ROLE_LABEL` from `src/constants/dashboard.ts` in both places.
4. **`DashboardFooterMenu` computes `fullName` twice.** `dashboard-footer-menu.tsx:51,88,101` — computed once and reused, fine; the `initials` derivation lacks a guard for `user === null` (falls back to `'Pengguna'` / email). Safe, but note.
...
5. **AlertDialog logout has correct Bahasa copy and semantics.** `dashboard-footer-menu.tsx:138-153` — title `Keluar dari akun?`, description, `Batal` cancel, destructive `Keluar` confirm; confirm calls `logoutUser()`, handles 401 as already-logged-out, then `markAnonymous()` + `navigate('/auth/login', { replace: true })`; non-401 errors toast and stay on dashboard.
...
- [x] Dropdown labels/routes and AlertDialog logout flow — `dashboard-footer-menu.tsx` (`Akun Saya` → `/dashboard/profile`, `Ganti Password` → `/dashboard/password`, `Keluar` → confirm → API → session clear → `/auth/login`).
...
**Historical — superseded.** Implementation matched the plan and spec; static verification was green and browser smoke was blocked because no Playwright runner/browser is installed. Since this report, the latent role fallback was removed (`role ? DASHBOARD_NAV_BY_ROLE[role] : []`), `ROLE_LABEL` was hoisted to `src/constants/dashboard.ts`, and the user menu was renamed `dashboard-footer-menu.tsx`. See `docs/reviews/sidebar-pr3-final-review.md` for current status.
...
2. ~~Resolve Important 1: remove the `role ?? 'member'` fallback~~ — done in the PR #3 follow-up.
3. ~~Cheap cleanups in the same pass: hoist `ROLE_LABEL` (Minor 3), tick plan checkboxes (Minor 7).~~ — `ROLE_LABEL` hoisted; plan checkboxes tracked in `docs/superpowers/plans/2026-09-20-sidebar-follow-up.md`.
